...
// Simple random token generator (temporary, can be replaced by JWT)
...
app.put('/api/orders/:id/status', async (req, res) => {
  try {
    ...
    if (order.customerEmail && order.customerEmail !== 'walkin@beanbox.com') {
      let emailSubject = '';
      let emailStatus = '';
      
      if (status === 'accepted') {
        emailSubject = 'Order Accepted - BeanBox Café';
        emailStatus = 'accepted';
      } else if (status === 'completed') {
        emailSubject = 'Order Ready for Pickup - BeanBox Café';
        emailStatus = 'completed';
      } else if (status === 'cancelled') {
        emailSubject = 'Order Cancelled - BeanBox Café';
